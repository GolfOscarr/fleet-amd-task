...
__device__ __forceinline__ void
  ...
  // 16-byte aligned when the buffer base is (docs/gpu/02-validation P2). o in [0, D_C), lse at D_C.
  constexpr int P_ROW = ((D_C + 1 + 3) / 4) * 4;
  // loads in flight per thread (docs/gpu/02-validation P6): the streaming knee measured on the MI300X
