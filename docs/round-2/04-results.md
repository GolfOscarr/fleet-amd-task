...
| E2 (`--nt-weights`) | the largest lever measured: 15.0 to 13.0 ms; `mla_attend` 215 to 150 us. `USE_NT_WEIGHTS=1` is the compile flag `-DMPK_NT_WEIGHT_LOADS` of the runtime: the stock linear, rmsnorm and silu kernels load their weights non-temporally, so the weight streams stop evicting the L2 and the attention finds its cache rows and its q in L2 more often | keep on; the same treatment for our kernels' streaming reads (the cache rows) is the next cheap experiment |
