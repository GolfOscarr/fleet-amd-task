...
| [`06-lessons.md`](06-lessons.md) | what went wrong and the fix, in three groups (missed by the preparation, on the clock, believed then measured false), and what the measurements say for the next round, in the order of the expected gain |
| [`07-summary.md`](07-summary.md) | one page: what was asked, what was reached (M4, the fault's cause and fix, the image), the time per token on the event clock against the design band, what it cost, what is open, where everything is |
...
| 2026-09-16 | session A and B on one 1x MI300X VM (`enc1-gpuvm015`, 149 minutes, $7.33): M4 reached (32 ids equal at 27 layers with the head); the fault named (`L0.gate_up`: the silu gang kernel reads 16 rows at batch 1) and fixed in the plan without a flag; the image pushed to GHCR; timings 12.3 ms per token baseline on the event clock, 9.6 ms with E2 and per-tile linears (15.0 and 10.3 ms as host means); the attention kernel is 34 us standalone and 146 to 215 us in the graph whatever the dispatch path, the prefetch depth or the rows per tile; the log in `03`, the numbers in `04`; branch `gpu/round-2` |
