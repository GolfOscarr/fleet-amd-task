...
| [`03-session-log.md`](03-session-log.md) | the log of the sessions: the timeline per plan row with every status line, the DECIDE rows with rule, measured value and choice, the nine failures and fixes, the artifacts |
| [`04-results.md`](04-results.md) | the numbers: the correctness evidence (M4), the fault's cause, the per-token time on three clocks against the baseline and the design band, the per-operator table, the traffic rows, what remains open |
