...
(`docs/gpu-experiments/01-bringup/`). The rule of this round: no minute of VM time goes to
...
| 7, 8, 9 layers fault deterministically before the first iteration; 2, 3, 4, 16, 27 run; 27 with the head faults only at 32 iterations | `docs/gpu-experiments/01-bringup/03-lessons-and-ideas.md` item 13 | M4 is blocked on one fault |
...
| Two sessions cost $12.56 on the 2x shape, 4.5 hours | `docs/gpu-experiments/01-bringup/04-session-log.md` | $27 on the 1x shape is about 9 hours |
...
- `docs/gpu-experiments/01-bringup/06-agent-guide.md` (the second GPU in three places),
