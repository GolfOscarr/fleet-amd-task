...
| Only the 2x MI300X VM was available, at $5.98 per hour | the second GPU is idle | see idea 7 below; round 2 runs on the 1x at $2.99 (`docs/gpu-experiments/02-validation/02-session-plan.md`) |
...
| `run_fleet.py --debug` aborts at graph registration | the per-layer snapshot copy (`copy_mi300` into the hidden-state dump) shares no tensor with the operator after it, and `register_mugraph` asserts `num_shared_tensors >= 1` for every consecutive pair (the chain dependency model of `03-synchronization.md`) | the snapshot must read `x_res` and write a buffer the next operator also names, or be issued as the last operator of the layer reading what the next layer reads; the growth curve over layers 2 to 26 waits on that harness fix (`05-next-session.md`, item 5) |
...
12. **The gang model is the batch-1 bottleneck, not the memory system.**
    ...
    Re-read on 2026-09-16 while preparing round 2 (`docs/gpu-experiments/02-validation`): the
...
13. **A graph of 7, 8 or 9 layers faults; 27 layers fault at the full
    ...
    Update 2026-09-16 (the round 2 preparation, `docs/gpu-experiments/02-validation/01-preparation.md`):
