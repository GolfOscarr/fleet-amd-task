...
| **F5. The half-merge fault** (L2, MIN-36) | three readings (the stock o_proj's input map against the merge tile's whole-tensor maps; the two graphs' task types; the tile form's store at `halves = 2`) and the fix with its suite row if one finds the defect; otherwise the locator rows for R4 and the readings attached to MIN-36. Done: the readings found nothing (`04-checklist.md`); `queue-h4` locates at 2 layers, `queue-h7` bisects the layer count only if 2 layers pass | the plan tests, the dry runs, the suite row, the offline compile | 2 h | R4 |
...
| **R3. The exec table and the merge** (L1, L3; F4's fix is verified by the first row) | F4, F6 | `queue-h3`: `--layers 2 --iters 1 --final --worker-timing compare`, `--layers 2 --iters 32 --final --worker-timing table`; `L start ktime nt` | the `[TASK_TIME2]` lines present and the exec per class in `report_table.md` (the GEMV linear's exec against the CK tile's 18 us of round 3); the merge's `ktime` line against 16.3 | RULE: a hang ends the row at the watchdog and MIN-35 stays open with the offline reading attached |
| **R4. The fault** (L2) | F5 | `queue-h4` at 2 layers, 45 s a row: `--layers 2 --iters 1 --final --no-gemv-linears` (the round-3 linears with the half merge, a configuration no row has run at 2 layers), the same cut after the first stock o_proj and after the first merge (`--stop-after L0.o_proj`, `--stop-after L0.mla_merge_uv`; the verdict is the run's rc), and `--merge-halves 1` as the control; then `queue-h7`, the bisect at 3, 5, 9 and 14 layers, only if the 2-layer row passed | the operator the fault sits in (the cut after the merge runs, the cut after o_proj faults: the stock o_proj after the tile merge; both fault: the merge or earlier; the 2-layer row passes: the first faulting layer count) and the halves' role, in the record | RULE: the rows only record; MIN-36 closes on the laptop when the located operator's defect is found and its fix passes a later row |
