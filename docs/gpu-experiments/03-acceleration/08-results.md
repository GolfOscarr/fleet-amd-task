...
| plus the attention as regular tasks: **the round's number** | `L27_head_it30/31/32_tile_at_fn1_fn2_fs_nt_mfma` | **4,599.5, 4,578.2, 4,589.3** | PASS (32 of 32) |
| the same, no event timing, `FWD_PASS` clock | `L27_head_it29_tile_at_fn1_fn2_fs_nt_mfma` | 4,584.0 (P95 4,634) | 29 of 29 |

2.08x faster than round 2 on the same clock; 2.0% above the 4.5 ms target.
...
| `--attend-tasks` with the MFMA kernel | 11:12 | 4,618 to 4,589; on |
| `--nt-streams` (O6) | S4 | within 0.2 us of plain standalone; not run in a graph |
| the fence and sleep knobs (I4) | S11 | see `07`, the knob rows |
