...
| **L6. Per-tile silu and combine** (C4, the first step of B2 and B3) | `moe_silu_mul_layer` with a grid of 66 tasks and `moe_mul_sum_add_layer` with 64, under `--tile-moe`; the imaps checked against the kernels' indexing (the runtime hands each task an offset pointer, as the per-tile linears showed) | the dry run; the wrappers' assertions; a test | 2 h | G5 |
