...
- [x] I4 the fence knobs: five defines in the runtime hunk of `new_tasks.patch` (`MPK_NO_COMPLETION_FENCE`: the release fence before the completion atomic, three sites; `MPK_NO_ACQUIRE_FENCE`: the acquire after the dependency load, two sites; `MPK_NO_BCAST_CAS` and `MPK_NO_LOCAL_CAS`: the two publish CAS loops of the scheduler queues as a store; `MPK_POLL_SLEEP=n`: the `s_sleep` constant of the poll and dependency loops, default 1), `run_fleet.py --runtime-flags=DEFINE` (repeatable, one define per option in the = form, so a queue row carries it and argparse does not read the dash as an option; suffix `_rf_<slug>`, the flags in the meta), the offline compile of each knob and the fence tables of the two fence knobs in `fences.txt`, `queue-c4.txt` (G4: the 2-layer graph with the compare, each knob alone, the sleep at 8, 32, 127)
- [ ] I5 the vLLM stage: skipped on 2026-09-17 (the user's decision: no time for the vLLM dependencies and the measurement); the 4.5 ms figure stays the recruiter's
- [x] I6 the clock helper: the queue's run step samples `amd-smi metric --clock` once a second into `clock.log` beside the run while the graph runs (only where `amd-smi` exists; never fails the row); `measure.py` reduces it to the median and maximum GFX clock (the median over the XCDs per sample) and the memory clock, in the report; a test on the format recorded in round 1
- [x] I7 `05-session-plan.md` (the rows of sessions C and D with minute marks, PASS text, AUTO or DECIDE and the rule, the thresholds T1 to T8 from round 2's numbers, the queue files, the helpers, the playbook, the fallbacks, the expected outputs; I5's row dropped) and `06-rehearsal.md` from `env/session/rehearse.sh` (43 queue rows and every stage expanded in DRY mode, exit 0, no guard message); the six session queues (`queue-c2`, `c5`, `c6`, `d7`, `d8`, `d9` beside `c3` and `c4`) with `harness/tests/test_queue_files.py` parsing every row and refusing duplicate run names; three `vm.sh` stages the plan needed (`kernels [variant]` for the nt and mfma suites, `ktime [variant]` for the standalone grids and the spin line, `tgcheck <run>` for the side operators' wiring, whose JSON the record excludes)

Status: I7 done 2026-09-17 (commit below): the plan does not use the GHCR image (no stage runs inside it; `setup` builds Fleet on the VM in about eight minutes, as round 2 measured), and `queue-d7b.txt` is written between the sessions from session C's answer. I4 and I6 done 2026-09-17 (commit below). `fences.txt` per knob, the worker: the default build has 4 agent-scope `buffer_wbl2 sc1` and 2 `buffer_inv sc1`; `MPK_NO_COMPLETION_FENCE` leaves 2 and 2 (the two completion sites of the union removed), `MPK_NO_ACQUIRE_FENCE` 4 and 0 (both acquire sites), nothing else moves; every knob variant compiles (seventeen variants and the host check exit 0). the knobs live in `new_tasks.patch` (every runtime hunk of ours is regenerated there; `gfx942.patch` is the day-1 port and stays as it is); the two CAS knobs are probes only (a plain store drops the in-order publish of the queue), which is why every G4 row carries the compare. A queue row is split on whitespace without shell quoting, found by the dry run of `queue-c4.txt`: hence one define per `--runtime-flags`, attached with `=` (a value starting with a dash is an option to argparse otherwise, the dry run's second finding). I1 to I3 done 2026-09-17 (commit below on `local/round-3`); what the source settled: the timing counters are `clock64` (the shader clock), hence the spin; the placement is not the plan's to choose (the prelaunch puts task index p on worker p modulo the worker count), so the ladder reads it back from every worker's line; the copy registration keeps its one parameter unless the spin is asked for, so the model's snapshots and probes are unchanged. Checks: 177 tests (three new), check_syntax 9 PASS, preflight 8 PASS, the offline compile of all twelve variants and the host syntax check exit 0. `fences.txt` regenerated: the agent-scope fences of the worker (`buffer_wbl2 sc1` 4, `buffer_inv sc1` 2) are unchanged; the system-scope counts of the printf hostcall paths rose (72 to 120, 48 to 80) with the spin's printf site now in the worker's union, a path taken only when a task spins.

## Double-check of the preparation (2026-09-17, before I7)

Mechanical, on the committed tree:

- the suite (179 tests), the syntax check (9) and preflight (8) fresh; the fork pristine after every patch flow
- the task-type table: values 185 to 194 unique across the whole enum; every type in the runtime's name table and the profiler map; the three gang types in the gang predicate and in every runtime list the stock gang types are in (attention and merge in four, the fused w2 in three: MoE gang tasks take no `n_tile_start`); the per-split attention and the expert prefetch in the `expert_offset` list
- every emitted call of the ten registrations matched by argument count against the kernel signature in `fleet/tasks/mi300` (the link only the VM build exercises otherwise); the input and output counts of `graph.cc` equal to each registration's and to the wrapper's tensor list
- 1,024 combinations of the eight plan flags at 1, 2 and 27 layers with and without the head: no exception, no chain violation, every method in `OUTPUT_ARGS`; a probe and a truncation at every operator of a 2-layer graph with every fusion and the prefetch; the ladder at the queue's 12 sizes
- the offline dispatcher covers every task type; the kernels table's task-type numbers equal the enum's; the operator counts in the docs equal the dry run's

By reading: the side-operator branch against the loop's invariants (`host_is_first` set before the host's step 2, the tasks appended in bid order behind the host's, the range of the host's last event extended, the map recorded for the printer), the per-split attention's index through the metadata, the MFMA kernel's fragment addresses against the layout test's formulas, the prefetch tasks' slot and part arithmetic against the mask's layout.

Found and fixed (commit below): a probe before `argmax_reduce` did not rewire its pair of inputs (the chain broke); the MFMA kernel copied an uninitialised prefetch buffer after the last pass (indeterminate values, never read); the offline dispatcher lacked the per-split attention type (its emitted call was never instantiated offline). Not re-read this pass, verified earlier by their disassembly: O1's router, O2's prologue, O6's loads. What only the VM can settle stays as listed in each item's VM row.
