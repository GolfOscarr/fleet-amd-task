...
| [`05-session-plan.md`](05-session-plan.md) | the two VM sessions row by row: the commands, the PASS text, AUTO or DECIDE with the rule and who decides, the thresholds from round 2, the queue files, the helpers, the playbook, the fallbacks, the expected outputs; the budget ($20.23: 2.5 hours each and a reserve) |
| [`06-rehearsal.md`](06-rehearsal.md) | every command of the two sessions expanded in DRY mode by `env/session/rehearse.sh`: the proof that each stage, helper and queue row exists and parses before the VM is billed |
