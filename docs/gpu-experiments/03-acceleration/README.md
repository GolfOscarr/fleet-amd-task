...
| 2026-09-17 | the VM session (`07`, `08`): 4,578 to 4,600 us per token on the event clock, 2.0% above the 4.5 ms target, ids equal; the event table's names corrected; the prep task per head, the router, merge and norm loads batched, the MFMA attention, the attention as regular tasks, the fusions; the prefetch off (slower), the fence knobs off (the completion fence is needed, the acquire-fence knob hangs); the record in `env/hw/20260917/`; branch `gpu/round-3` |
