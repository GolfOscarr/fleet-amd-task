...
| [`07-session-log.md`](07-session-log.md) | the VM session of 2026-09-17 minute by minute: the rows as run, what the measurements said in order (the event table's names off by one, the fusions neutral, prep at 4 ms, four latency-bound kernels), the plan defects and traps found and fixed, the rules applied, what was not done |
| [`08-results.md`](08-results.md) | the numbers: 9,575 to 4,571 to 4,600 us per token on the event clock (4,584 by `FWD_PASS`), ids equal; where the time goes per operator before and after; the kernels standalone; every lever's verdict; the measurement corrections; what remains for 4.5 ms and below |
| [`09-lessons.md`](09-lessons.md) | every approach with its hypothesis, evidence and verdict; the lessons on measurement, kernels, the runtime and the process; the next round ranked; the corrections to the earlier documents |
...
| 2026-09-17 | the VM session (`07`, `08`, `09`): 4,571 to 4,600 us per token on the event clock, 2.0% above the 4.5 ms target, ids equal; the event table's names corrected; the prep task per head, the router, merge and norm loads batched, the MFMA attention, the attention as regular tasks, the fusions; the prefetch off (slower), the fence knobs off (removing either fence fails the step-0 compare; the CAS and sleep knobs not run); the record in `env/hw/20260917/`; the VM deleted at minute 144 ($7.12); branch `gpu/round-3`; `01` to `05` carry a read-after note each, `06` regenerated from the split queues |
| 2026-09-18 | the fourth round run in [`../04-kernels/`](../04-kernels/README.md) (`09-session-log.md`, `10-results.md`): the GEMV linears, the deeper router and the half merge take the token from this round's 4,571 to 4,600 us to 4,262 to 4,341, ids equal, below the 4.5 ms target; the stream probe puts the task-shaped reads at 2.25 TB/s, the next round's lever |
