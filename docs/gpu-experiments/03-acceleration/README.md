...
| 2026-09-18 | the fourth round prepared in [`../04-kernels/`](../04-kernels/README.md): the CK linears, the router and the merge as kernels of our own behind flags, the session plan `07-session-plan.md` with round 3's numbers as its thresholds; the ranked list of `09` is taken up in that order but rank 1 (the per-XCD completion hierarchy), which its G2 row sizes |
