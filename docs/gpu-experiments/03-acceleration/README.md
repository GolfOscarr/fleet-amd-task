...
| 2026-09-17 | laptop parts of O0, O1, O2, O5, O6, O3 and O7 done on `local/round-3` (each a commit; the flags `--fuse-norm2`, `--fuse-silu`, `--fuse-norm1`, `--probe-before`, `--nt-streams`, `--mfma-attend`, all default off until the VM validates them): with the three fusions the graph is 246 operators instead of 326; the MFMA attention's lane arithmetic is tested by emulation, the instruction runs first on the VM; O8 and the instruments remain; no VM |
