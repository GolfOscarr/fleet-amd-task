...
| K8, the first batch before the prologue is free | with hipcc 7.0 a batch that is live across the prologue and reloaded inside the loop is not coalesced with the loop's own: the wave-1 kernels measured 250 (GEMV), 238 (router) and 244 (merge) registers standalone against 172, 170 and 166 without the pre-load, and the worker union 208 AGPRs against 100 (`05`, the third convention; `env/offline_gfx942/gemv_probe/order_probe.cu`) | the pre-load forms were measured and removed (the final check of 2026-09-18); the norm's round trip is paid, about 1 to 2 us per fused task |
| K1's loads may be guarded per row | a branch around a row's loads is a basic block of its own, and the scheduler keeps only that block's loads in flight: the guarded GEMV waited `vmcnt(3) (2) (1) (0)` per row | rows past the range are clamped, not skipped |
...
- **The multiply.** Per row per lane: 32 conversions (a shift or a mask)
  ...
  count; the halving butterfly is the form to build: at each of log2(rows)
  steps a lane exchanges half of its live row sums with a partner lane
  and adds (8 + 4 + 2 + 1 = 15 shuffles for 16 rows), then the remaining
  single sums cross the last lane bits (two steps): 17 shuffles against
  96, and lane l ends holding row l's sum for a 32-byte store. About 2 us of
