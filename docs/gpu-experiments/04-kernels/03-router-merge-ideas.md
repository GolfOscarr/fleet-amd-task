...
| R1 and M2, the first batch before the prologue and `W_uv` before the partials, are free | not with this compiler: a batch live across a prologue costs about 70 registers (`01`, the double-check; `05`, the third convention): the router 238 against 170, the merge 244 against 166 | both behind `ROUTER_PRELOAD` and `MERGE_W_PRELOAD`, off by default, for the VM's A/B; the estimates of R1 (2 us) and M2 (3 to 4 us) are paid unless the A/B pays for them |
