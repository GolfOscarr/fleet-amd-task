...
| **G3. The w2 form** | L3 | `--layers 2 --iters 1 --gemv-linears --fuse-silu compare`, then the `it32 wt table` (the GEMV multiply is the pushed header's default path of the fused w2; there is no `--gemv-w2` flag) | 0 FAIL, ids equal; the w2 gap and exec against 23.9 | DECIDE: on if below 16 us; otherwise the CK multiply is one define away: `--runtime-flags=-DMPK_W2_CK_TILE` |
...
| **G5. The stream probe** (M7) | L6 | `--graph stream --kb 304 --gang` (whole 4 KB rows: `--kb` is a multiple of 4, so w13's 305 KB tile is 304), `--kb 152` regular 96 tasks, `--kb 256` regular 296; every operator reads its own tensor (74 to 90 MB each, so the 256 MB memory-side cache holds nothing an operator re-reads) | GB/s per XCD (the gang rows) and per device (the regular rows); the 152 and 304 KB rows are 38 and 76 rows, a partial last batch of 8, so the kernel requests 5% more lines than the byte count (the clamped re-reads hit L2; the 256 KB row is exact) | the ceiling for S1 and every gang linear; written into `08-results.md` as the machine's number |
| **G6. The head** (S3) | L5 | `--layers 2 --head --iters 32 --gemv-linears --worker-timing table` (a timing row: a 2-layer graph's logits cannot match the 27-layer reference, so the head form's correctness is G7's ids and compare), then the same with `--head-grid 320` (`--linear-grid` names qkva and o_proj only, and 320 divides neither) | the head's chunk events against 141 us | DECIDE: the head's grid |
