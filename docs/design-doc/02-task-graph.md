...
```python
mpk = mi.PersistentKernel(mode="online", num_workers=296, num_local_schedulers=8,
                          num_remote_schedulers=0, max_seq_length=1056,
                          max_num_batched_requests=1, max_num_batched_tokens=1,
                          max_num_pages=1, page_size=1056, eos_token_id=-1,
                          meta_tensors={...ten tensors...})

x = mpk.embed_layer(input=None, weight=W_embed, output=x_res, grid_dim=(1,1,1),
...
for l in range(27):
    ...
    if l == 0:
        ...
        mpk.gang_linear_with_residual_layer(act, W_down, x_res, x_res, tile_n=32, output_stride=2048)
    else:
        mpk.moe_router_layer(h, W_gate[l], (topk_w, routing, mask), block_dim=(256,1,1))   # NEW
...
mpk.argmax_reduce_layer((amax_v, amax_i), None, grid_dim=(1,1,1), block_dim=(256,1,1),
                        output_to_tokens=True)                                # variant: writes tokens[step+1]
```
...
After the prelaunch rewrite the scheduler dispatches task at graph position
`p` to worker `(p - 2) mod 296`, and gang tasks are broadcast to the workers
of that worker's XCD (`docs/fleet/03-runtime.md`). With workers placed
round-robin (`w` on XCD `w mod 8`), eight consecutive gang tasks land on eight
distinct XCDs, and which XCD serves which `bid.x` rotates with the position.
...
| `mla_attend` | splits `{x, x+8, x+16, x+24, x+32}` for XCD `x` (the last exists only on XCD 0); 5 workers busy per XCD, 33 in all |
| `mla_merge_uv` | heads `{2x, 2x + 1}`; 2 workers per XCD |
| `gang_moe_w13`, `gang_moe_w2` | active expert `mask[x]`: with 8 active, one expert per XCD; 44 (W13) or 32 (W2) tiles over 37 workers |
| 1-task ops | worker `(p - 2) mod 296`; wherever that is |
...
| Tile decode | `split = xcd_local_tile * 8 + bid.x` for tile `t` in `[0, 5)`; positions `[32 split, 32 split + 32)` clipped to `step + 1`; a split entirely past `step` writes `lse = -inf` and returns |
...
| | |
|---|---|
| Inputs | `partials`; `W_uv [16, 512, 128]` BF16 (a view of `kv_b_proj`); `step` |
| Outputs | `attn [1, 2048]` BF16, columns `[128 h, 128 h + 128)` for head `h` |
| Tile decode | `h = 2 bid.x + t` for tile `t` in `{0, 1}` |
| Work | over the `ceil((step + 1) / 32)` live splits: `m = max m_j`, `l = sum l_j exp(m_j - m)`, `acc = sum acc_j exp(m_j - m)`; `o = acc / l` `[512]`; `attn[h] = o @ W_uv[h]` `[128]`, FP32 accumulation, 128 KiB of `W_uv` per tile |
...
| `argmax_reduce` | emit `runtime_config.tokens + runtime_config.step[0] + 1` as the output pointer | online mode does not copy `output_tokens` into `tokens` (`docs/fleet/04-repo-map.md`) |
...
| gang linear SiLU: `(N_gu / 8 / tile_n)` even | same | 22528 / 8 / 64 = 44 |
| CK pipeline: `K % 256 == 0` (linear, W13); `K % 128 == 0` (W2) | `linear_kernel_ck`, `gang_moe_*` | 2048, 11264, 512 (`mla_merge_uv`), 1408 |
| `argmax_partial`: `V % slices == 0` | `argmax_partial_layer` | 102400 / 50 = 2048 |
