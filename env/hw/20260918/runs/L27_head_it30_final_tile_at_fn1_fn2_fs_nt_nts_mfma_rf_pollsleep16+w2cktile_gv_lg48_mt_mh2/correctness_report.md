...
The run made 30 iterations: the boundaries other than the cache rows and the first token were dumped from iteration 29 and are not comparable against the reference's step 0 (NOT_COMPARABLE below); the output ids and the route log carry the verdict.
...
| `head.B15.logits` | B15 | logits | - | - | - | - | exact | **NOT_COMPARABLE** dumped from iteration 29; the reference has no ref_boundaries_step29 |
| `head.B16.token` | B16 | exact | - | - | - | - | exact | **PASS** fleet 25 ref 25 |

Output ids: **PARTIAL**, 30 of 32 matched.
Route log: **PASS**, 30 steps compared, 62 mismatching (step, layer) pairs: 31 ties, 31 cascades, 0 disagreements (the tie rule, tol 0.0144 of the larger weight).
