...
- [ ] **M4** End-to-end 32-token decode — the 27-layer graph with the head produces the reference's tokens at 1 and 2 iterations ([25], then [25, 16228]); at 32 iterations it faults with an illegal memory access (2026-09-15), with and without event timing and with larger runtime queues; 8 layers with the head fault at 8 iterations already, before the first iteration reports; without the head 27 layers run 4 iterations and 2 layers run 32; the fault is in the head path at larger configured iteration counts (`docs/hw-collection/03-lessons-and-ideas.md`, item 14)
- [ ] **M5** FP8 (stretch)
