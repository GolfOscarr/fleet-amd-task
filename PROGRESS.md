...
- [ ] **M4** End-to-end 32-token decode — the 27-layer graph with the head produces the reference's tokens at 1 and 2 iterations ([25], then [25, 16228]); at 32 iterations it faults with an illegal memory access (2026-09-15), with and without event timing and with larger runtime queues; the frontier with the head is non-monotonic in the layer count (2, 4, 16 pass; 8 fails at every iteration count; 27 passes at 2 and fails at 32), deterministic, before the first iteration reports; without the head 27 layers run 32 iterations; a plan-derived size or index, to be named by the verbose runtime (`docs/hw-collection/03-lessons-and-ideas.md`, item 14)
- [ ] **M5** FP8 (stretch)
