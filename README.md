...
`docs/gpu-experiments/05-final/`, branch `local/round-5`) is the final and
light stage, prepared on the laptop and waiting for its one session: no
new kernel; the round-4 stack as one flag (`run_fleet.py --final`), the
compare made green by construction (the route log's tie rule with the
gate's softmax weights in the reference, the iteration-aware
boundaries), the head's event count as a plan argument, the
worker-timing hang fixed in the patch (no device printf on the timing
build), the half-merge fault read and left to a 2-layer locator, the
merge's standalone 5 us read offline and recorded; the session plan
(`05-session-plan.md`, about 68 minutes on the whole $4.69 balance under
a hard stop) and its results page (`07-final-numbers.md`, drafted with
round 4's numbers) are ready.

| | |
|---|---|
| Documentation | 5 discovery sets (41 files) + the design set (15 files, 1 script) + the five GPU sets under `docs/gpu-experiments/` (the bring-up of 2026-09-15, the validation of 2026-09-16, the acceleration of 2026-09-17 with its log, results and lessons, the kernels prepared on 2026-09-18: the ideas for the GEMV linear and for the router and merge, the two splits, the local preparation, the checklist, the session plan, the rehearsal, its log, results and lessons; the final stage of 2026-09-18: the ideas, the split, the preparation, the checklist, the session plan, the rehearsal, the results page drafted) |
...
| Next agent | the final stage, prepared on the laptop and waiting for its VM session: the plan [`docs/gpu-experiments/05-final/05-session-plan.md`](docs/gpu-experiments/05-final/05-session-plan.md), the checklist [`04-checklist.md`](docs/gpu-experiments/05-final/04-checklist.md), the results page to fill [`07-final-numbers.md`](docs/gpu-experiments/05-final/07-final-numbers.md); the set's ideas and split: [`docs/gpu-experiments/05-final/`](docs/gpu-experiments/05-final/README.md); round 4's outcome and what remains: [`docs/gpu-experiments/04-kernels/10-results.md`](docs/gpu-experiments/04-kernels/10-results.md), its lessons [`11-lessons.md`](docs/gpu-experiments/04-kernels/11-lessons.md), session log [`09-session-log.md`](docs/gpu-experiments/04-kernels/09-session-log.md) and plan [`07-session-plan.md`](docs/gpu-experiments/04-kernels/07-session-plan.md); how to reach the GPU and run on the VM: [`docs/gpu-experiments/01-bringup/06-agent-guide.md`](docs/gpu-experiments/01-bringup/06-agent-guide.md); round 2 in one page: [`docs/gpu-experiments/02-validation/07-summary.md`](docs/gpu-experiments/02-validation/07-summary.md), then its log, numbers and lessons in [`docs/gpu-experiments/02-validation/`](docs/gpu-experiments/02-validation/README.md) |
...
| Open problems | 5 major open (MAJ-8: the runtime's per-task completion cost, not what bounds the GEMV linears by round 4's grid sweep; MAJ-7 resolved in round 3 as a measurement defect), 18 minor (MIN-35 fixed in the patch by round 5, the VM's first timing row the check; MIN-36 read on the laptop, its 2-layer locator on the VM), 34 resolved ([`OPEN-PROBLEMS.md`](OPEN-PROBLEMS.md)) |
