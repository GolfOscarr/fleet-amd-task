...
| Next agent | how to reach the GPU and run on the VM: [`docs/gpu-bringup/06-agent-guide.md`](docs/gpu-bringup/06-agent-guide.md); round 2 in one page: [`docs/round-2/07-summary.md`](docs/round-2/07-summary.md), then its log, numbers and lessons in [`docs/round-2/`](docs/round-2/README.md) |
| Session image | pushed 2026-09-16 as `ghcr.io/golfoscarr/fleet-amd-task:20260916` (25 GB, private) by the `image` stage of round 2; the Dockerfile in [`env/docker/README.md`](env/docker/README.md); the runs of round 2 in [`docs/round-2/03-session-log.md`](docs/round-2/03-session-log.md) and their numbers in [`04-results.md`](docs/round-2/04-results.md) |
...
| Measured (round 2, 2026-09-16) | **9.6 ms per token, 104 tok/s** steady state with E2 and per-tile linears; 12.3 ms with the gang linears; the attention kernel 34 us standalone against 146 us in the graph ([`docs/round-2/04-results.md`](docs/round-2/04-results.md)) |
...
docs/
  ...
  round-2/           the second round: the preparation on the laptop,
                     the two-session plan on a 1x MI300X, its log, results,
                     lessons and the one-page summary (07)
...
| Correctness evidence at every boundary | method in [`docs/deepseek-v2-lite/08-correctness.md`](docs/deepseek-v2-lite/08-correctness.md); the evidence in [`docs/round-2/04-results.md`](docs/round-2/04-results.md) and the reports under `env/hw/20260916/runs/` |
| Profiling commands and results | plan in [`docs/mi300x/06-profiling.md`](docs/mi300x/06-profiling.md); the commands in `env/session/` and the results in [`docs/round-2/04-results.md`](docs/round-2/04-results.md) |
| Milestone reached, remaining fallbacks | [`PROGRESS.md`](PROGRESS.md), [`docs/round-2/07-summary.md`](docs/round-2/07-summary.md) |
| Known failures | [`OPEN-PROBLEMS.md`](OPEN-PROBLEMS.md), [`docs/round-2/06-lessons.md`](docs/round-2/06-lessons.md) |
| Recommended next steps | [`docs/round-2/06-lessons.md`](docs/round-2/06-lessons.md) (measured, in the order of the gain), [`docs/acceleration/04-technique-ledger.md`](docs/acceleration/04-technique-ledger.md) |
